pipeline {
  ...
  stages {
    ...
    stage('Checkout Code') {
      steps {
        script {
          def repoUrl = 'https://github.com/Mini-Soccer/payment-service.git'
    ...
    stage('Commit and Push Changes') {
      steps {
        script {
          ...
          git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB_CREDENTIALS_PSW}@github.com/Mini-Soccer/payment-service.git
    ...
    stage('Deploy to Remote Server') {
      steps {
        script {
          def targetDir = "/home/maomao/Documents/adit/buildwithangga/mastering-microservice-golang/payment-service"
          ...
          ssh -o StrictHostKeyChecking=no -i ${SSH_KEY} ${USERNAME}@${HOST} '
            if [ -d "${targetDir}/.git" ]; then
                ...
                git clone -b "${TARGET_BRANCH}" git@github.com:Mini-Soccer/payment-service.git "${targetDir}"
                cd "${targetDir}"
            fi
