...
```
payment-service
    L clients                        → Contains the client for calling other service
...
### perintah build dockerfile
```bash
docker build --no-cache -t mini-soccer-project/payment-service:latest .
```
...
### konfirmasi semua containers didalam 1 services ini berjalan
```bash
docker compose ps
```
